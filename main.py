...
def cargar_datos():
    ...
    if not os.path.exists(ARCHIVO):
        print("Info: No se encontró archivo.")
        return
...
def agregar_producto():
    ...
    for p in productos:
        if p.nombre.lower() == nombre.lower():
            print(f"El producto '{p.nombre}' ya existe. Stock actual: {p.stock}")
            cantidad = validar_numero("Cantidad a reabastecer: ", int, 1)
            p.stock += cantidad
...
def guardar_reporte_ventas():
    ...
    nombre_archivo = os.path.join(carpeta, f"ventas_{fecha_hoy}.txt")
    total = 0
...
def mostrar_reporte():
    ...
    confirmacion = input("\n¿Desea exportar el reporte a un archivo? (s/n): ").strip().lower()
    if confirmacion == "s":
        guardar_reporte_ventas()
    else:
